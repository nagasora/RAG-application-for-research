...
1. 親エージェントは `Sol` を使用し、プロジェクトマネージャー兼統合責任者として要件、依存関係、担当ファイル、受入条件を整理する。
...
3. 調査・実装・レビューのサブエージェントは、原則として `Terra` を使用する。高コストなモデルへの切り替えは、親が必要性を説明できる場合だけ行う。
4. アプリ概要、利用ガイド、設計文書などの文書作成は `Luna` を優先する。実行環境で `Luna` を選択できない場合は `Terra` へフォールバックし、親の `Sol` が内容とコード整合性を最終確認する。
5. 独立した読み取り作業は並列化する。並列実装は担当ファイルが重ならない場合に限る。
6. `backend/app/models.py`、API 契約、`frontend/app/page.tsx` などの共有境界は、一人の担当を決めてから編集する。
7. 各サブエージェントへの指示には、目的、非目標、担当ファイル、参照する `CI-xxx`、受入条件、検証コマンド、他の変更を巻き戻さないことを含める。
8. サブエージェントは担当範囲、変更ファイル、検証結果、未確認事項を親へ返し、親が差分を統合して最終検証する。
9. 実装後は別担当の `quality_reviewer` が読み取り専用レビューを行い、親が重大指摘を解消または残存理由を記録する。
10. 小さな一ファイル修正や、委譲コストが作業量を上回る場合は単独で進めてよい。
