...
def test_worker_role_grants_only_ingestion_tables():
    assert set(WORKER_GRANTS) == {
        "papers", "chunks", "ingestion_jobs", "paper_pages", "document_elements",
        "chunk_embeddings", "embedding_jobs",
    }
    assert "DELETE" not in WORKER_GRANTS["papers"]
    assert "INSERT" not in WORKER_GRANTS["ingestion_jobs"]
    assert WORKER_GRANTS["embedding_jobs"] == ("SELECT", "INSERT", "UPDATE")
    assert "DELETE" not in WORKER_GRANTS["chunk_embeddings"]
